...
function ReissueGenerateOrReject() {
    ...
    const handleReject = async () => {
        try {
            await axios.post(`${apiList.generateReissueCard}${userDetails[0]._id}`,{
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
    ...
    const handleGenerate = async () => {
        try {
            console.log(token)
            userDetails[0].userDebitCardDetails.userDebitCardStatus = 'active';
            const response = await axios.post(`${apiList.generateReissueCard}${userDetails[0]._id}`,{
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
                    ...
                    {userDetails && userDetails.length > 0 && (
                        <div className="user-details-container container">
                            ...
                            <div className="account_details col-sm-12">
                                <div className='account_values'>
                                    <label>Account Number:</label>
                                    <input className='account_input' value={userDetails[0].accountNumber} readOnly />
                                </div>
                                <div className='account_values'>
                                    <label>Account Name:</label>
                                    <input className='account_input' value={userDetails[0].firstname} readOnly />
                                </div>
                                <div className='account_values'>
                                    <label>Account Type:</label>
                                    <input className='account_input' value={userDetails[0].openaccount} readOnly />
                                </div>
                                <div className='account_values'>
                                    <label>Bank Branch:</label>
                                    <input className='account_input' value={userDetails[0].bankBranch} readOnly />
                                </div>
                                <div className='account_values'>
                                    <label>State:</label>
                                    <input className='account_input' value={userDetails[0].currentAddress.state} readOnly />
                                </div>
                                <div className='account_values'>
                                    <label>City:</label>
                                    <input className='account_input' value={userDetails[0].currentAddress.city} readOnly />
                                </div>
                                <div className='account_values'>
                                    <label>Village:</label>
                                    <input className='account_input' value={userDetails[0].currentAddress.village} readOnly />
                                </div>
                                <div className='account_values'>
                                    <label>Pincode:</label>
                                    <input className='account_input' value={userDetails[0].currentAddress.pincode} readOnly />
                                </div>
                            </div>
                            <div className='debit-details col-sm-12'>
                                <p>Debit Card Details:</p>
